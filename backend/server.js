...
/* you can also use "import/dotenv/config". if you do so then you dont need to call dotenv.config() because it automatically gets called */
...
/* Helmet is used by the server to send instructions to the browser—via HTTP response headers—about how the browser should handle and interpret the data it receives.
Even though it only touches responses it helps with the following below through
setting response headers :
It reduces XSS attack surfaces
It prevents clickjacking
It enforces HTTPS usage (via HSTS)
It limits where scripts/resources can load from (CSP) */
app.use(helmet());

/* form-data is plain text in the form of key_value pairs : username=alice&password=1234 and most data sent to the server through CRUD requests which are not sent using a form are sent as key and value pairs in JSON format:  {"username":"alice","password":"1234"}
form-data is parsed using app.use(express.urlencoded({ extended: true }));  and json is parsed using app.use(express.json()) */
/* The result of parsing in both cases is a JS object which is attached to req.body */
app.use(express.json());

app.use(cors());

/* logs requests(a request status code, its type(GET, POST etc)) */
app.use(morgan("dev"));

/* Arcjet for Bot detection. Rate limiting. Email validation. Attack protection. Data redaction. A developer-first approach to security. */
...
/* app.post("/", (req, res, next) => {
	console.log("testing backend next");
	res.send("testing frontend next");
	//next();
}); */
